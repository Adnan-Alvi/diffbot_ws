...
from diffbot_msgs.srv import GetTransform
...
class SimpleTfKinematics(Node):
    def __init__(self):
        ...
        self.static_tf_transform_ = StaticTransformBroadcaster(self)
        ...
        self.oreintation_increment_ = quaternion_from_euler(0, 0, 0.05)

        self.tf_buffer_ = Buffer()
        self.tf_listener_ = TransformListener(self.tf_buffer_, self)

        self.static_transform_stamped_.header.stamp = self.get_clock().now().to_msg()
        self.static_transform_stamped_.header.frame_id = "diffbot_base"
        self.static_transform_stamped_.child_frame_id = "diffbot_top"
        ...
        self.static_tf_transform_.sendTransform(self.static_transform_stamped_)
        
        self.get_logger().info("Publish static transform between %s and %s" % (self.static_transform_stamped_.header.frame_id, self.static_transform_stamped_.child_frame_id))
    ...
    def timerCallback(self):
        ...
        self.dynamic_transform_stamped_.child_frame_id = "diffbot_base"
        ...
        q = quaternion_multiply(self.last_orientation_, self.oreintation_increment_)
        ...
        if(self.rotations_counter_ >= 100):
            self.oreintation_increment_ = quaternion_inverse(self.oreintation_increment_)
            self.rotations_counter_ = 0
